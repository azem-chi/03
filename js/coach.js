...
${(()=>{
  ...
  log.forEach(e=>{
    ...
    ids.forEach(id=>{
      ...
      if(ex?.muscles) ex.muscles.split(/[،,]/).forEach(m=>{const k=m.trim();if(k) muscleGroups[k]=(muscleGroups[k]||0)+1;});
    });
  });
...
${_T('IDs أنشأتها:','Coach-created IDs:','IDs créés:')} ${(S.customExercises||[]).filter(e=>e._coachCreated).map(e=>_escHtml(e.id)+' ('+_escHtml(e.nameAr)+')').join(', ') || '—'}
${_T('التمارين المخصصة:','Custom exercises:','Exercices personnalisés:')} ${(S.customExercises||[]).map(e=>_escHtml(e.id)+' ('+_escHtml(e.nameAr)+')').join(', ') || '—'}
